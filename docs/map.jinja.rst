...
We will see a quick setup to configure the ``samba-ad`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``samba-ad`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``samba-ad`` formulas under ``/srv/salt/samba-ad/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/samba-ad/parameters/dns:domain/
    ln -s dns:domain /srv/salt/samba-ad/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/samba-ad/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/samba-ad/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``samba-ad`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``samba-ad`` formulas under ``/srv/salt/samba-ad/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/samba-ad/parameters/roles
...
We create a configuration for the role ``samba-ad/server`` in ``/srv/salt/samba-ad/parameters/roles/samba-ad/server.yaml``:
...
We create another configuration for the role ``samba-ad/client`` in ``/srv/salt/samba-ad/parameters/roles/samba-ad/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles samba-ad/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles samba-ad/client
...
.. code-block:: console
    ...
    mv /srv/salt/samba-ad/parameters/dns:domain/  '/srv/salt/samba-ad/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as samba-ad with context %}

    test-does-nothing-but-display-samba-ad-as-json:
      test.nop:
        - name: {{ samba-ad | json }}
...
Here is an example based on `template-formula/samba-ad/config/file.sls`_:
...
.. code-block:: sls
    ...
    samba-ad-config-file-file-managed:
      file.managed:
        ...
        - mode: 644
        - user: root
        - group: {{ samba-ad.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            samba-ad: {{ samba-ad | json }}

This ``sls`` file expose a ``samba-ad`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ samba-ad['winner'] }}
...
.. _template-formula/samba-ad/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/samba-ad/config/file.sls
